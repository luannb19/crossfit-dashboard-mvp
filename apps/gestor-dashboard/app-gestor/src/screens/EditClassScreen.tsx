...
function formatStartAtForInput(iso: string): string {
...
export default function EditClassScreen({ route, navigation }: Props) {
  ...
  const loadClass = useCallback(async () => {
    ...
    try {
      ...
      const cls = JSON.parse(raw) as {
        title: string;
        startAt: string;
        capacity: number;
      };
      setTitle(cls.title);
      setStartAt(formatStartAtForInput(cls.startAt));
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      if (!res.ok) {
        setError((data.error as string) ?? "Erro ao salvar aula.");
        return;
      }
      navigation.goBack();
    } catch (err) {
  ...
  if (error && !title) {
    return (
      <View style={styles.center}>
        ...
        <TouchableOpacity style={styles.submitBtn} onPress={loadClass}>
    ...
    >
      ...
      >
        ...
        <TouchableOpacity
          ...
          disabled={saving}
        >
          <Text style={styles.submitBtnText}>Marcar presenças</Text>
        </TouchableOpacity>
        <TouchableOpacity
